...
```bash
# Download
bash scripts/extract_audio.sh path/to/video-channel-clip.mp4
...
### Video Note (Xiaohongshu / WeChat Video Channels)
...
> **Note:** WeChat Video Channels files should include the "Key Timestamps" section for easy key-frame jumping when transcript timestamps are available.
...
| `download_media.sh` fails | Platform restriction / geo-block | For Video Channels, upload the file directly; for Xiaohongshu, retry with a logged-in browser/cookies only when permitted |
